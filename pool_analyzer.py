...
# Streamlit App with Advanced Visualizations
...
if st.sidebar.button("Calculate"):
    with st.spinner("Calculating..."):
        ...
        # Advanced Visualization 1: Candlestick Chart for Asset 1 Price Trend
        dates = pd.date_range(start="2024-01-01", end="2025-03-17", freq="D")
        price_data = pd.DataFrame({
            "Date": dates,
            "Open": initial_price_asset1 + np.random.normal(0, 10000, len(dates)),
            "High": initial_price_asset1 + np.random.normal(0, 15000, len(dates)),
            "Low": initial_price_asset1 + np.random.normal(0, 15000, len(dates)),
            "Close": np.linspace(initial_price_asset1, current_price_asset1, len(dates)) + np.random.normal(0, 5000, len(dates))
        })
        price_data["Close"].iloc[-1] = current_price_asset1
        ...
        fig_candlestick.update_layout(
            title="Asset 1 Price Trend (Candlestick)",
            xaxis_title="Date",
            yaxis_title="Price ($)",
        ...
        # Advanced Visualization 2: 3D Surface Plot for Impermanent Loss Sensitivity
        price_change_asset1 = np.linspace(-50, 50, 50)
        price_change_asset2 = np.linspace(-50, 50, 50)
        il_values = np.zeros((len(price_change_asset1), len(price_change_asset2)))
        for i, change1 in enumerate(price_change_asset1):
            for j, change2 in enumerate(price_change_asset2):
                current_price_asset1 = initial_price_asset1 * (1 + change1 / 100)
                current_price_asset2 = initial_price_asset2 * (1 + change2 / 100)
                il_values[i, j] = calculate_il(initial_price_asset1, initial_price_asset2, current_price_asset1, current_price_asset2, investment_amount)
        fig_surface = go.Figure(data=[
            go.Surface(
                x=price_change_asset1,
                y=price_change_asset2,
                z=il_values,
                colorscale="Viridis",
                showscale=True
            )
        ...
        fig_surface.update_layout(
            title="Impermanent Loss Sensitivity to Price Changes",
            scene=dict(
                xaxis_title="Asset 1 Price Change (%)",
                yaxis_title="Asset 2 Price Change (%)",
        ...
        # Advanced Visualization 3: Sankey Diagram for Profit and Loss Breakdown
        ...
        il_loss = investment_amount - pool_value
        apy_gain = pool_value * (apy / 100)  # Assume 1 year for simplicity
        final_value = pool_value + apy_gain
        fig_sankey = go.Figure(data=[
            go.Sankey(
                node=dict(
                    ...
                    label=["Initial Investment", "Pool Value (After IL)", "IL Loss", "APY Gain", "Final Value"],
                    color=["#1f77b4", "#ff7f0e", "#d62728", "#2ca02c", "#9467bd"]
                ),
                link=dict(
                    source=[0, 0, 1, 1],
                    target=[1, 2, 3, 4],
                    value=[pool_value, il_loss, apy_gain, pool_value],
                    color=["#1f77b4", "#d62728", "#2ca02c", "#9467bd"]
                )
            )
        ])
        fig_sankey.update_layout(title_text="Profit and Loss Breakdown", font_size=12, template="plotly_dark")
        st.plotly_chart(fig_sankey, use_container_width=True)

        # Advanced Visualization 4: Radar Chart for Risk Factors Comparison
        categories = ["Protocol Risk", "TVL Decline Risk", "Pool Share Risk", "Volatility Risk"]
        scores = [protocol_risk_score, tvl_decline, pool_share, 50]  # Using volatility score of 50 as placeholder
        fig_radar = go.Figure()
        ...
        # Original Projections Section (Updated with Matplotlib replaced by Plotly)
        ...
        # Replace Matplotlib with Plotly for Pool Value Over Time
        fig_line = go.Figure()
        fig_line.add_trace(go.Scatter(x=time_periods, y=future_values, mode="lines+markers", name="Pool Value", line=dict(color="#1f77b4")))
        fig_line.add_hline(y=investment_amount, line_dash="dash", line_color="red", annotation_text="Initial Investment", name="Initial Investment")
        fig_line.update_layout(
            title="Projected Pool Value Over Time",
            xaxis_title="Months",
            yaxis_title="Value ($)",
            template="plotly_dark"
        )
...
# Backup of Original Visualization Section (Comment this out to revert)
"""
# Original Visualization Section (Replaced by Advanced Visualizations)
st.subheader("Projected Pool Value Based on Yield, Impermanent Loss, and Price Changes")
st.write(f"**Note:** The initial projected value reflects the current market value of your liquidity position, adjusted for price changes and impermanent loss, not the cash invested (${investment_amount:,.2f}).")
time_periods = [0, 3, 6, 12]
future_values = []
