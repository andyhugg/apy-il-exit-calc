...
def calculate_protocol_risk_score(apy: float, tvl_decline: float, current_tvl: float) -> tuple[float, str, str]:
    base_score = 0
    # APY-based risk
    if apy < 10:
        base_score += 40
    ...
    adjusted_score = min(base_score, 100)
    ...
    # Generate message
    if category == "Low":
        message = f"✅ Protocol Risk: Low ({adjusted_score:.0f}%). Minimal risk due to high yield, stable TVL, or large pool size."
...
def check_exit_conditions(initial_investment: float, apy: float, il: float, tvl_decline: float,
                         ...
                         current_tvl: float, risk_free_rate: float, months: int = 12, 
    ...
    protocol_risk_score, protocol_risk_message, protocol_risk_category = calculate_protocol_risk_score(apy, tvl_decline, current_tvl)
    ...
    with col2:
        ...
        if aril < 0:  # Loss Scenario
            ...
            aril_note = f"Your pool’s effective return (ARIL) is {aril:.1f}%, above the Hurdle Rate of {hurdle_rate:.1f}% (risk-free rate + 6% inflation) but below the target of {target_aril:.1f}% to justify risk. Returns are marginal for the risk taken. Evaluate if this aligns with your investment goals."
