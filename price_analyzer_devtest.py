...
if calculate:
    if asset_price == 0 or initial_investment == 0:
        ...
        with st.expander("Key Metrics", expanded=False):
            ...
            tooltip_text = (
                f"Your asset’s growth ({asset_return:.2f}x) is better than BTC (1.25x) and stablecoins ({stablecoin_return:.2f}x). "
                f"It also beats the safe target. Stay in. If risks are high, add stablecoins or switch to BTC."
            )
